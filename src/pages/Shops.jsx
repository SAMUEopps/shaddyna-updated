        ...
        <div className="w-[85%] md:w-[90%%] sm:w-[90%] lg:w-[90%] h-full mx-auto">
          <div className={`md:block hidden ${!filter ? "mb-6" : "mb-0"}`}>
            <button
              ...
              className="text-center w-full py-2 px-3 bg-blue-400 text-white"
            >
              Filter Product
            </button>
          </div>
          <div className="w-full flex flex-wrap">
            ...
            >
              ...
              <div className="p-3 shadow flex flex-col gap-5 bg-white my-2">
                ...
                <Range
                  step={5}
                  min={Math.min(priceRange.low, priceRange.high - 1)} // Ensure min is always less than max
                  max={Math.max(priceRange.high, priceRange.low + 1)} // Ensure max is always greater than min
                  values={state.values}
                  ...
                  renderThumb={({ props }) => (
                    <div className="w-[15px] h-[15px] bg-blue-400 rounded-full" {...props} />
                  )}
                />

                <div>
                  <span className="text-blue-400 font-bold text-lg">
                    Ksh{Math.floor(state.values[0])} - Ksh{Math.floor(state.values[1])}
                  </span>
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {tagOptions.map((item, i) => (
                    <span
                      ...
                      key={i}
                      className="bg-blue-400 hover:bg-orange-500 transition-all duration-300 cursor-pointer text-white font-medium px-2 py-0.5 text-sm"
                    >
                      {item.label}
                    </span>
                  ))}
              ...
              <div className="p-2 w-full bg-white shadow my-2">
                <button onClick={Reset} className="bg-blue-400 text-white w-full px-5 py-2 font-medium">
                  Reset
                </button>
              </div>
            </div>
