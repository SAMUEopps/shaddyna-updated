    ...
    <Container className="">
      <nav className="rounded-xl p-4 flex flex-row justify-between">
        <div className="flex flex-row gap-x-4 items-center relative">
          ...
          />*
          <div
            className="text-4xl font-bold text-black-300 cursor-pointer"
            onClick={() => window.open("/", "_self")}
          >
            Shaddyna
          </div>
        ...
        <div className="hidden md:flex flex-row gap-x-2 relative">
...
const Header = () => {
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg p-3 flex justify-between items-center z-50">
        {user && Object?.keys(user)?.length > 0 && (
          <button
            className="p-2 rounded-secondary hover:bg-slate-100 transition-colors"
            onClick={() => window.open("/dashboard", "_self")}
